...
|---|---|---|---|---|
| `CAN_ID_VCU_TORQUE_CMD` | VCU → inverter | `0x100` | bytes[0..1] = int16 torque, LE, 1 unit/LSB; bytes[2..7] = 0 | gated to 0 on any torque-inhibiting fault (§5.7) |
| `CAN_ID_INVERTER_HB` | inverter → VCU | `0x200` | (heartbeat) | feeds the CAN command watchdog |
| `CAN_ID_AMS_DCBUS` | AMS → VCU | `0x300` | byte[0] = % of accumulator | for the pre-charge ≥90 % gate (§5.4) |

## Torque command encoding (placeholder)
- Signed 16-bit, little-endian in bytes [0..1]; remaining bytes zero.
- Range clamped by `control/torque` to `[0, TORQUE_MAX_CMD]` (no reverse, §5.7).
- Confirm the inverter's torque resolution, endianness, sign convention, and any
  enable/state bytes before relying on this.

## To finalize (see discrepancies.md)
